...
| M2b | Chạy `supabase/phase_a_commerce_migration.sql` | Supabase → SQL Editor | ⏳ **chờ user** (Phase A) |
| M3 | Kiểm tra Realtime đã bật cho `public.notifications` | Supabase → Database → Replication | ✅ **ĐÃ BẬT** — verify bằng `python _verify_realtime.py` |
| M4 | Set `DATABASE_POOL_URL` (Supabase pooler, port 6543, transaction mode) | biến môi trường / `.env` | ⏳ tuỳ chọn nhưng khuyến nghị |
| M5 | Tạo merchant account PayOS + lấy `PAYOS_CLIENT_ID` / `API_KEY` / `CHECKSUM_KEY` | PayOS Dashboard | ⏳ Phase A |
| M6 | Đăng ký webhook URL với PayOS (PayOS sẽ gửi ping xác minh) | PayOS Dashboard | ⏳ Phase A |
...
| **A — Commerce** | `bots`/`carts`/`orders`, two-phase checkout qua asyncpg, PayOS client + webhook, cron expire + polling fallback, tab admin đối soát | PayOS (M5/M6) + M4 khuyến nghị | ⬜ **CHƯA** ← *bước tiếp theo* |
